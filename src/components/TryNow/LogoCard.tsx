...
const LogoCard = ({ logo, title, onClick, selected }: LogoCard) => {
  return (
    ...
    >
      <motion.div className="w-24 h-24 bg-yellow-300 flex items-center justify-center rounded-full" whileHover="hover" variants={logoCardAnimations.logoContainer}>
        <img src={logo} alt={title} className="w-12 h-12" />
      </motion.div>

      <h3 className="text-xl font-bold text-blue-600 dark:text-blue-400 mt-4">{title}</h3>
