...
const PostCard: React.FC<PostCardProps> = React.memo(({
    ...
    if (isLoading || !post) {
        return (
            <div
                className={`bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden ${viewMode === 'list' ? 'flex' : viewMode === 'magazine' ? 'h-full' : ''
                    }`}
                style={{ animation: 'pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite' }}
            >
                <div className={`${viewMode === 'list' ? 'w-1/3' : viewMode === 'magazine' ? 'h-48' : 'h-56'}`}>
                    <div className="w-full h-full bg-gray-200 dark:bg-gray-700" />
                </div>
                <div className={`${viewMode === 'list' ? 'w-2/3' : ''} px-6 py-12 space-y-4`}>
    ...
    return (
        <motion.article
            className={`bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer overflow-hidden group ${viewMode === 'list' ? 'flex' : ''
                }`}
        ...
        >
            <div className={`${viewMode === 'list' ? 'w-1/3' : ''} relative`}>
                <div className={`${viewMode === 'list' ? 'h-full' : 'h-56'} overflow-hidden`}>
                    {post.image ? (
                        <img
                            src={post.image}
                            alt={post.title}
            ...
            <div className={`${viewMode === 'list' ? 'w-2/3' : ''} p-6 flex flex-col justify-between`}>
                <div>
                    <h3 className={`font-bold text-gray-800 dark:text-gray-200 mb-3 line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors ${viewMode === 'list' ? 'text-xl' : 'text-lg'
                        }`}>
                        {post.title}
                    </h3>
