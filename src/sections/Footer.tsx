...
const Footer = () => {
  return (
    <>
      <footer className="bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-950 text-gray-700 dark:text-gray-300 py-10 sm:py-16 px-4 sm:px-6 relative">
        {/* Decorative top border */}
        <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 opacity-70 dark:opacity-100"></div>
              ...
              <ul className="space-y-2 sm:space-y-3">
                {resourceLinks.map((link) => (
                  <li key={link.to}>
                    <Link
                      to={`${link.to}`}
                      className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-white transition-all duration-300 flex items-center group"
                    >
                      <span className="w-0 group-hover:w-2 h-2 bg-blue-500 dark:bg-blue-400 rounded-full mr-0 group-hover:mr-2 transition-all duration-300"></span>
                      {link.text}
                    </Link>
                  </li>
                ))}
                  ...
                  <li key={link.to || link.href}>
                    {link.href ? (
                      <a
                        ...
                        className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-white transition-all duration-300 flex items-center group"
                      >
                        <span className="w-0 group-hover:w-2 h-2 bg-purple-500 dark:bg-purple-400 rounded-full mr-0 group-hover:mr-2 transition-all duration-300"></span>
                        {link.text}
                      </a>
                    ) : (
                      <Link
            ...
            <div className="sm:col-span-2 lg:col-span-6">
                  ...
                  >
                    <img
                      ...
                      className="relative z-10 transition-transform duration-300
                        filter brightness-0 dark:invert opacity-70 dark:opacity-90 
              ...
              <a
                ...
                className="inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2.5 sm:py-3 
                  bg-gray-200/70 dark:bg-gray-800/50 hover:bg-gray-300 dark:hover:bg-gray-700 
                  rounded-lg transition-all duration-300 transform hover:-translate-y-1 
                  hover:shadow-lg hover:shadow-gray-400/30 dark:hover:shadow-gray-800/30 
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 sm:gap-4 py-6 sm:py-8 border-t border-gray-300 dark:border-gray-800">
            {quickLinks.map((link) => (
              <Link
                key={link.to}
                to={`${link.to}`}
                className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-white transition-all duration-300 text-xs sm:text-sm"
              >
                {link.text}
              </Link>
            ))}
          ...
          <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-500 pt-4 sm:pt-6 border-t border-gray-300 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4">
            ...
            <p className="text-center sm:text-right">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-gray-700 dark:text-gray-400 hover:text-blue-600 dark:hover:text-white transition-colors duration-300 underline decoration-dotted"
              >
